...
my $obj = new_ok('Genealogy::Obituary::Lookup');

diag("Genealogy::Obituary::Lookup->search test case created by https://github.com/nigelhorne/App-Test-Generator");
...
my %input = (
	'age' => { max => 120, min => 0, optional => 1, type => 'integer' },
	'first' => { max => 100, min => 1, optional => 1, type => 'string' },
	'last' => { matches => qr/^[\w\-]+$/, max => 100, min => 1, type => 'string' },
	'middle' => { max => 100, min => 1, optional => 1, type => 'string' }
);
...
foreach my $case (@{fuzz_inputs()}) {
	...
	if(my $status = (delete $case->{'_STATUS'} || delete $output{'_STATUS'})) {
		if($status eq 'DIES') {
			dies_ok { $result = $obj->search($input); } sprintf($mess, 'dies');
		} elsif($status eq 'WARNS') {
			warnings_exist { $result = $obj->search($input); } qr/./, sprintf($mess, 'warns');
		} else {
			lives_ok { $result = $obj->search($input); } sprintf($mess, 'survives');
		}
	} else {
		lives_ok { $result = $obj->search($input); } sprintf($mess, 'survives');
	}
